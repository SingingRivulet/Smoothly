...
void building::addDefaultBuilding(){
    def_shader = driver->getGPUProgrammingServices()->addHighLevelShaderMaterialFromFiles(
                ...
                "../shader/building_default.vs.glsl", "main", irr::video::EPST_PS_1_1);
    {
        auto ptr  = new conf;
        ptr->id   = -1;
        config[-1]= ptr;
    ...
    }
    {
        ...
        buffer->Vertices.push_back(video::S3DVertex(-5,-0.5,-5, -1,-1,-1, clr, 0, 1));
        buffer->Vertices.push_back(video::S3DVertex(5 ,-0.5,-5,  1,-1,-1, clr, 1, 1));
        buffer->Vertices.push_back(video::S3DVertex(5 ,0,-5,  1, 1,-1, clr, 1, 0));
        buffer->Vertices.push_back(video::S3DVertex(-5,0,-5, -1, 1,-1, clr, 0, 0));
        buffer->Vertices.push_back(video::S3DVertex(5 ,9.5,5,  1,-1, 1, clr, 0, 1));
        buffer->Vertices.push_back(video::S3DVertex(5 ,10,5,  1, 1, 1, clr, 0, 0));
        buffer->Vertices.push_back(video::S3DVertex(-5,10,5, -1, 1, 1, clr, 1, 0));
        buffer->Vertices.push_back(video::S3DVertex(-5,9.5,5, -1,-1, 1, clr, 1, 1));
        buffer->Vertices.push_back(video::S3DVertex(-5,10,5, -1, 1, 1, clr, 0, 1));
        buffer->Vertices.push_back(video::S3DVertex(-5,0,-5, -1, 1,-1, clr, 1, 1));
        buffer->Vertices.push_back(video::S3DVertex(5 ,9.5,5,  1,-1, 1, clr, 1, 0));
        buffer->Vertices.push_back(video::S3DVertex(5 ,-0.5,-5,  1,-1,-1, clr, 0, 0));
        ...
        ptr->texture = driver->getTexture("../../res/tree_trunk.tga");
        ptr->haveBody = true;
        ptr->bodyShape.init("+b0 5 0 0.924 0 0 0.383 5 0.5 7.07");
        ptr->canBuildOn = true;
        ptr->fetchBB=ptr->mesh[0]->getBoundingBox();
        ptr->useAutoAttach = true;
        ptr->autoAttach.deltaHor = 10;
    ...
    }
    {
        ...
        buffer->Vertices.push_back(video::S3DVertex(5 ,0   ,-5,  1, 1,-1, clr, 1, 0));
        buffer->Vertices.push_back(video::S3DVertex(-5,0   ,-5, -1, 1,-1, clr, 0, 0));
        buffer->Vertices.push_back(video::S3DVertex(5 ,4.5 ,5,  1,-1, 1, clr, 0, 1));
        buffer->Vertices.push_back(video::S3DVertex(5 ,5   ,5,  1, 1, 1, clr, 0, 0));
        buffer->Vertices.push_back(video::S3DVertex(-5,5   ,5, -1, 1, 1, clr, 1, 0));
        buffer->Vertices.push_back(video::S3DVertex(-5,4.5 ,5, -1,-1, 1, clr, 1, 1));
        buffer->Vertices.push_back(video::S3DVertex(-5,5   ,5, -1, 1, 1, clr, 0, 1));
        buffer->Vertices.push_back(video::S3DVertex(-5,0   ,-5, -1, 1,-1, clr, 1, 1));
        buffer->Vertices.push_back(video::S3DVertex(5 ,4.5 ,5,  1,-1, 1, clr, 1, 0));
        buffer->Vertices.push_back(video::S3DVertex(5 ,-0.5,-5,  1,-1,-1, clr, 0, 0));
        ...
        ptr->texture = driver->getTexture("../../res/tree_trunk.tga");
        ptr->haveBody = true;
        ptr->bodyShape.init("+b0 2.5 0 0.973 0 0 0.23 5 0.5 5.6");
        ptr->canBuildOn = true;
        ptr->fetchBB=ptr->mesh[0]->getBoundingBox();
        ptr->useAutoAttach = true;
        ptr->autoAttach.deltaHor = 10;
